...
#include <unistd.h>
#include <ctype.h> //Used for whitespace checking.
...
typedef struct Command {
    ...
    char **args;
    struct Command *next;
    uint32_t stdin_fd; //Handled in execution
    uint32_t stdout_fd; //Handled in execution
    char *redir_in_path;
    char *redir_out_path;
...
** -- NULL if no command could be found on the path, //TODO: WHAT HAPPENS IF NULL IS PRINTED?
...
/*
** The following is a helper function for free_command
** input is heap allocated array of pointers to heap allocated strings. Pointer array ends at NULL pointer
 */
