  ...
  try {
    {
      ...
      NetworkInterfaceTestHarness test { "Large Test #2", local_eth, Address( "4.3.2.1", 0 ) };
      ...
      // STEP 2: Wait a bit, then send back the responses to the ARP requests. 
      test.execute( Tick { 2000 } );

      for (int i = 0; i < host_count; i++) {
        // skip the first three, which should have their requests expired at this point. 

        if (i >= 3) {
          // the rest will receive a reply which they should use to complete the arp entry 
          test.execute( ReceiveFrame {
            make_frame(
              remote_eth[i],
              local_eth,
              EthernetHeader::TYPE_ARP, 
              serialize( make_arp( ARPMessage::OPCODE_REPLY, remote_eth[i], 
                                    next_hop_string[i], local_eth, "4.3.2.1" ) ) ),
            {} } );

          // the frames that were waiting should be sent now 
