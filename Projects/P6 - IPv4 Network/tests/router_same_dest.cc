...
void network_simulator()
{
  ...
  cout << green << "\n\n Sending multiple packets toward the same destination ..." << normal
       << "\n\n";

  {
    ...
    // this somewhat tests the PA1 as well 
    dgram_sent = network.host( "applesauce" ).send_to( network.host( "doughnut" ).address() );
    dgram_sent.header.ttl--;
    dgram_sent.header.compute_checksum();
    network.host( "doughnut" ).expect( dgram_sent );
